...
class ProjectRunner:
    ...
    def run_queries(self, query_list, random_command):
        ...
        for query in tqdm(query_list):
            ...
            and_op_no_skip_sorted = [x[0] for x in reversed(sorted(traversal, key = lambda x: -1*x[1]))]
            # and with skip
            ...
            and_op_skip_sorted = [x[0] for x in reversed(sorted(traversal, key = lambda x: -1*x[1]))]

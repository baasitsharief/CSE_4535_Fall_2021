...
class ProjectRunner:
    ...
    def run_queries(self, query_list, random_command):
        ...
        for query in tqdm(query_list):
            ...
            and_op_no_skip_sorted = list(reversed([x[0] for x in sorted(traversal, key = lambda x: x[1])]))
            # and with skip
            ...
            and_op_skip_sorted = list(reversed([x[0] for x in sorted(traversal, key = lambda x: x[1])]))
